...
from recipes.models import ChatMessage, Recipe, RecipeDraftSuggestion, User
...
class AIChatbotGetViewTest(AIChatbotViewTestCase):
    ...
    def test_get_chatbot_shows_existing_messages(self):
        ...
        ChatMessage.objects.create(
            user=self.user, role=ChatMessage.Role.USER, content="I want a pasta recipe"
        )
        ChatMessage.objects.create(
            user=self.user,
    ...
    def test_get_chatbot_shows_publish_button_when_draft_exists(self):
        ...
        self.assertContains(
            response, reverse("ai_chatbot_publish", kwargs={"draft_id": draft.id})
        )
...
class AIChatbotMessageViewTest(AIChatbotViewTestCase):
    ...
    def test_post_message_creates_draft_and_messages(self, mock_keys, mock_suggest):
        ...
        response = self.client.post(
            self.message_url,
            data=json.dumps(
                {"prompt": "Make me pasta", "dietary_requirements": "none"}
            ),
            content_type="application/json",
        )
    ...
    def test_post_message_handles_service_error(self, mock_keys, mock_suggest):
        ...
        error_msg = ChatMessage.objects.filter(
            user=self.user, role=ChatMessage.Role.ASSISTANT
        ).first()
...
class AIChatbotPublishViewTest(AIChatbotViewTestCase):
    ...
    def setUp(self):
        ...
        self.publish_url = reverse(
            "ai_chatbot_publish", kwargs={"draft_id": self.draft.id}
        )
    ...
    def test_publish_forbidden_for_wrong_user(self):
        ...
        response = self.client.post(
            self.publish_url, data=json.dumps({}), content_type="application/json"
        )
...
class AIChatbotClearViewTest(AIChatbotViewTestCase):
    ...
    def test_clear_removes_chat_messages(self):
        ...
        # Create messages
        ChatMessage.objects.create(
            user=self.user, role=ChatMessage.Role.USER, content="Test message"
        )
        ChatMessage.objects.create(
            user=self.user, role=ChatMessage.Role.ASSISTANT, content="Response"
        )
    ...
    def test_clear_removes_unpublished_drafts(self):
        ...
        RecipeDraftSuggestion.objects.create(
            user=self.user, prompt="Test", status=RecipeDraftSuggestion.Status.DRAFT
        )
        ...
        self.assertEqual(
            RecipeDraftSuggestion.objects.filter(
                user=self.user, status=RecipeDraftSuggestion.Status.DRAFT
            ).count(),
            0,
        )
    ...
    def test_clear_json_response(self):
        ...
        response = self.client.post(
            self.clear_url, data=json.dumps({}), content_type="application/json"
        )
...
class AIChatbotIntegrationTest(AIChatbotViewTestCase):
    ...
    def test_full_workflow_message_to_publish(self, mock_keys, mock_suggest):
        ...
        response = self.client.post(
            publish_url, data=json.dumps({}), content_type="application/json"
        )
