...
class AuthorRecipesViewTest(TestCase):
    ...
    def test_author_recipes_loads(self):
        ...
        response = self.client.get(reverse('author_recipes', kwargs={'author_id': self.author.id}))
    ...
    def test_author_recipes_filters_by_search(self):
        ...
        response = self.client.get(
            reverse('author_recipes', kwargs={'author_id': self.author.id}) + '?search=First'
        )
        self.assertIn('recipes', response.context)
        recipes = response.context['recipes']
    ...
    def test_author_recipes_filters_by_dietary_requirement(self):
        ...
        self.recipe1.save()
        response = self.client.get(
            reverse('author_recipes', kwargs={'author_id': self.author.id}) + '?dietary_requirement=vegan'
        )
        self.assertIn('recipes', response.context)
        recipes = response.context['recipes']
        self.assertEqual(len(recipes), 1)
    ...
    def test_author_recipes_sorts_correctly(self):
        ...
        self.recipe2.save()
        response = self.client.get(
            reverse('author_recipes', kwargs={'author_id': self.author.id}) + '?sort_by=popularity'
        )
        self.assertIn('recipes', response.context)
    ...
    def test_author_recipes_with_nonexistent_author(self):
        ...
        response = self.client.get(reverse('author_recipes', kwargs={'author_id': 99999}))
