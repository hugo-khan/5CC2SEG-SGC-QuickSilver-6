...
class CommentDeleteViewTest(TestCase):
    ...
    def test_redirects_if_not_logged_in(self):
        ...
        response = self.client.get(reverse('comment_delete', kwargs={'pk': self.comment.pk}))
    ...
    def test_commenter_can_delete_comment(self):
        ...
        response = self.client.post(
            reverse('comment_delete', kwargs={'pk': comment_id}),
            follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertFalse(Comment.objects.filter(pk=comment_id).exists())
        self.assertRedirects(response, reverse('recipe_detail', kwargs={'pk': recipe_pk}))
