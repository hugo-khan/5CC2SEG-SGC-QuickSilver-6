...
class ReportCommentViewTest(TestCase):
    def setUp(self):
        self.reporter = User.objects.create_user(
            username="@janedoe", password="pass", email="jane@example.com"
        )
        self.comment_author = User.objects.create_user(
            username="@petrapickles", password="pass", email="petra@example.com"
        )
        ...
        self.comment = Comment.objects.create(
            recipe=self.recipe, user=self.comment_author, text="This is a test comment"
        )
    ...
    def test_login_required(self):
        ...
        response = self.client.post(
            self.url, {"comment_id": self.comment.id, "reason": "Spam"}
        )
    ...
    def test_create_report(self):
        ...
        has_success_message = any(
            "submitted" in str(m).lower() or "thank" in str(m).lower()
    ...
    def test_prevent_self_report(self):
        ...
        has_error_message = any(
            "own comment" in str(m).lower() or "cannot report" in str(m).lower()
            for m in messages_list
        )
    ...
    def test_prevent_duplicate_report(self):
        ...
        # Check info message
        messages_list = list(get_messages(response.wsgi_request))
        has_info_message = any(
            "already reported" in str(m).lower() for m in messages_list
        )
    ...
    def test_invalid_form_shows_error(self):
        ...
        messages_list = list(get_messages(response.wsgi_request))
        has_error_message = any(
            "valid reason" in str(m).lower() or "reason" in str(m).lower()
            for m in messages_list
        )
        self.assertTrue(has_error_message)
