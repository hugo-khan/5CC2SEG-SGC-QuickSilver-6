...
class CommentReportModelTest(TestCase):
    ...
    def test_create_report(self):
        report = CommentReport.objects.create(
            comment=self.comment, reporter=self.user, reason="Offensive"
        )
    ...
    def test_prevent_duplicate_report(self):
        CommentReport.objects.create(
            comment=self.comment, reporter=self.user, reason="Spam"
        )
        with self.assertRaises(Exception):
            CommentReport.objects.create(
                comment=self.comment, reporter=self.user, reason="Spam again"
            )
