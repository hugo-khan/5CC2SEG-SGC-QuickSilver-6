...
class TestAppConfigCoverage(TestCase):
    ...
    def test_asgi_config_coverage(self, mock_get_asgi):
        ...
        with mock.patch.dict("os.environ", clear=True):
            ...
            self.assertEqual(
                os.environ.get("DJANGO_SETTINGS_MODULE"), "recipify.settings"
            )
...
class TestProjectSettings(TestCase):
    ...
    def test_google_oauth_enabled_logic(self):
        ...
        with mock.patch.dict(os.environ, mock_env_full, clear=True):
            ...
            self.assertEqual(
                recipify.settings.SOCIALACCOUNT_PROVIDERS["google"]["APP"]["client_id"],
                "test_id_full",
            )
            self.assertEqual(
                recipify.settings.SOCIALACCOUNT_PROVIDERS["google"]["APP"]["secret"],
                "test_secret_full",
            )
